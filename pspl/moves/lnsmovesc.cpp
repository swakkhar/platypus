/*!
	@file moves/LNSMoveSC.cpp
	@brief The implementation file for LNSMoveSC class.
	@details This is the implementation file for LNSMoveSC class.
	@author Swakkhar Shatabda swakkhar.shatabda@nicta.com.au
	@author M.A.Hakim Newton hakim.newton@nicta.com.au
	@date 05.08.2012 QRL NICTA www.nicta.com.au
*/

#include "pspl/moves/lnsmovesc.hpp"
...
LNSMoveSC::LNSMoveSC(LNSMoveSC const & that):Move(that)
{
    WatchError
...
LNSMoveSC const & LNSMoveSC::operator= (LNSMoveSC const & that)
{
    WatchError
...
*/
void LNSMoveSC::compute(Conf & theConf, Pos const thePos)
{
    //WatchError
    mChanges.clear();
    ...
    for(Idx tIdx1=0;tIdx1<aPositions.itemCount();++tIdx1)
    {
        if(theConf.isPosSideChain(aPositions[tIdx1]))
            prior[tIdx1]=Lattice::l().Direction(theConf.Coord(aPositions[tIdx1])-theConf.Coord(aPositions[tIdx1]-Protein::p().Length()/2));
        else
    ...
    while(1)
    {
        ...
        for(;tIdx < aPositions.itemCount(); ++tIdx)
        {
            ...
            if(!theConf.isPosSideChain(thisPos)) // if backbone
            {
            ...
            else // member thepos is a side chain
            {
                lastPos=thisPos-Protein::p().Length()/2; // find the backbone
                // also need to set the lastpoint to the backbone - its a tricky one

                if(posDict.findBll(lastPos))
                {
                    lastPoint=tChange.DestinationByPos(lastPos);
                    newPoint = lastPoint + Lattice::l().DirVec(generator[tIdx]);
                    //      cout << "last " <<lastPoint.Comp(0) << " " << lastPoint.Comp(1) << " " << lastPoint.Comp(2) << " " << endl;
                }
                else
                ...
                else
                {
                    ...
                    // its already generated as a neighbor to the backbone, so now safely add
...
bool LNSMoveSC::next(block1<Dir,kmm> &generator)
{
    Dir carry=1;
...
bool LNSMoveSC::skip(block1<Dir,kmm> &generator, Idx pos)
{
...
void LNSMoveSC::addPosition(Pos tPos)
{
    Warn(tPos>=Protein::p().Length(), eInvalidIndex);
    //cout << tPos << endl;
    aPositions.insertMem(tPos);
}
bool LNSMoveSC::alreadyAdded(Pos tPos)
{
    for(Idx tIdx=0;tIdx<aPositions.itemCount();++tIdx)
    {
...
void LNSMoveSC::reset()
{
    aPositions.clear();
}
bool LNSMoveSC::equal(block1<Dir,kmm> &generator, block1<Dir,kmm> &prior)
{
